...
describe('isValidGroupJid', () => {
  it('accepts group jids ending in @g.us', () => {
    ...
    assert.equal(isValidGroupJid(' 120363-456789@g.us '), true)
  })

  it('rejects empty, DMs, typos, and other suffixes', () => {
    assert.equal(isValidGroupJid(''), false)
    assert.equal(isValidGroupJid('   '), false)
    assert.equal(isValidGroupJid('@g.us'), false)
    assert.equal(isValidGroupJid('x@g.us'), false)
